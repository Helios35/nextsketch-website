...
- Not a portfolio site — the page itself is the credibility play. **CHANGED** — the "selected work" grid is **no longer RETIRED**: decision-log #16 (2026-08-24) brought `#work` back as the proof band, first section under the hero, because nothing on the page was demonstrating proof of work. The principle holds — it is one band on the page, not a portfolio site. **CHANGED again (#39, #43):** per-project case study routes exist since 2026-09-12 and Mascot's is a full page since 2026-09-18 (`03-site-architecture.md`); the site is still not a portfolio site, and the modal is still the only lead path.
...
- Not a multi-page site — one page plus the modal. **CHANGED** — the site is **five static pages plus one dynamic segment** since decision-log #23 (2026-08-25), **#30** (2026-08-28) and **#39** (2026-09-12): the scrolling home page, `/pricing`, the two service routes `/services/product` and `/services/agentic-system`, the `/work` case study grid, and one `/work/<slug>` route per case study, prerendered from content (Mascot's a full page since #43, the rest placeholders until their material lands). The principle holds (no sprawl, no CMS — #41 says so in as many words — every page static, the modal still the only lead path); the literal count does not.
