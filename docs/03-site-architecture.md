...
**Version:** 2.6 · **Date:** 2026-09-18 · **Status:** Active — the site is **five static pages plus one dynamic segment**: the home page, `/pricing`, **two service routes** under `/services/`, the **`/work` case study grid** and **`/work/<slug>` per case study**, Mascot's a full page since #43 (decision-log #22–#26, **#30**, **#39–#41**)
...
/work                      — the case study grid: every study in content,
                             behind the home band's "View all" (static, ink) [NEW — #39]
/work/[slug]               — one route per case study, prerendered at build
                             from src/content/case-studies/ (static, ink);
                             a full page where the study has one (Mascot,
                             #43), the layout-final placeholder otherwise    [NEW — #39, #43]
...
**CHANGED again — #39–#41 (2026-09-12).** The case studies are routes: **`/work`** is the grid of every study in content and **`/work/<slug>`** is one route per study, **prerendered at build** from `src/content/case-studies/` (`generateStaticParams`; `dynamicParams` is `false`, so `/work/does-not-exist` returns the site 404 without a render). Both are static HTML and sell nothing, so **decision #8 still stands untouched**. The home page's proof band is the doorway: its "View all" lands on `/work` and every card lands on its own study — and that is the only thing about the home page that changed (the built home HTML differs from the previous build in those five destinations and nothing else). **`/work` and `#work` are a character apart and mean different things (#40):** the nav's Work item is the band, and the grid is reached from the band's control, not from the nav. Every href on the new routes is root-relative, built through `ROUTES.work` and `workHref` in `src/lib/types.ts`, never hand-written. **No CMS (#41):** the content is the repo, one module per study, and imagery follows the existing `/public/work/` convention with heavy media hosted externally by URL. The routes shipped **layout-final and empty** under #39; since **#43** (2026-09-18) a study with a page renders it (Mascot today) and the rest render the placeholder.
...
**CURRENT (as-built).** `SiteNav` and `SiteFooter` are both live and mount **per page** (`page.tsx`, `pricing/page.tsx`, both service routes, `work/page.tsx` and `work/[slug]/page.tsx`), not in `src/app/layout.tsx`, so the 404 keeps its own light paper surface. *(This section previously recorded them as RETIRED and unmounted — that was true of the 2026-06-14 pivot and was left stale when Redesign Unit 02 remounted them. Corrected here.)*
...
**The service routes are not nav items, and that is deliberate (#30).** The item set stays **six** — the nav is settled (#22, #24, #26) and this unit does not reopen it. The service pages are reached from the `#services` cards on the home page, where each card's **name** is now a link to that service's route, opening it at the top (#30, corrected 2026-08-30 — the block anchors skipped the hero), while the card's existing CTA still opens the modal on that service. The footer maps `NAV.items`, so it is unchanged too. Consequence, recorded rather than resolved: **a visitor on a service route cannot reach the other service route from the chrome** — only via `/#services`. Adding them to the nav is a separate owner call.

**`/work` is not a nav item either, and neither are the case study routes (#40).** The nav's Work item still means the home band (`/#work`); `/work` is reached from the band's "View all", and each case study from its card on the band or its tile on the grid. The two are one character apart in the URL bar on purpose, and neither is a mistake. Adding `/work` to `NAV.items` is a separate owner call.

**Every destination is root-relative** so it resolves from `/pricing`, from `/services/*` and from `/work/*` as well as `/` (#23, #30, #39), including both wordmark targets. With scripting off, the nav bar is a lockup and an inert burger — the tab row was its no-JS surface — but the footer's six anchors are plain `<a>` on every page, so nothing becomes unreachable (#22).
...
|---------|-----|----------------------|
| **Selected Work** (`#work`) — **CHANGED, live (decision-log #16; the doorway to `/work`, #39)** | **Prove the work is real — the conversion-recovery band, first below the hero; leads the nav (#21). Its "View all" lands on `/work` and every card on its own case study route (#39)** | **None in Messaging Kit. Band copy in `src/content/work.ts`; the studies themselves in `src/content/case-studies/`, one module each (#41) — owner-supplied names, links and screenshots delivered 2026-08-24; summaries DRAFT in brand voice (Rule 4.3)** |
...
**The case study routes — CHANGED, live (decision-log #39–#41, 2026-09-12).** `/work` is the grid: the `/pricing` chrome (page lockup, glow, nav, footer, metadata), an intro band reusing the proof band's own eyebrow, headline and supporting line, and every study as the band's card in its `tile` variant, on a column ladder chosen from the count so no tile is ever stranded alone on a row (`work-grid.tsx`). `/work/<slug>` is one route per study. **A study with a page renders it (#43, 2026-09-18):** `/work/mascot` is the owner's own composition on the design system — header row and hero-scale headline, two renders sinking behind an elevated panel with the mascot stepping out over its edge, the panel (a render beside the summary, then "What did we do?" answered with the proof line and the on-device screens), an annotated hardware band, a closing render, the other studies as tiles, the service routes' close — with the page's content in named slots beside the study's card in its module, registered by slug. **A study without one renders the layout-final placeholder** it has had since #39: the same chrome, the study's name and summary, a line saying the detail is coming, a route back to `/work` and a link out to the study's published page; that is the other three today. **Nothing narrative ships that the owner did not supply** — no results, metrics, client names or quotes (Rule 4.3). Content: `src/content/case-studies/` (one module per study, #41) and the page strings in `src/content/work.ts`, all DRAFT pending owner approval. As-built: build-notes 29 and 30.
