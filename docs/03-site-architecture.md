...
**Version:** 2.5 · **Date:** 2026-09-14 · **Status:** Active — the site is **five static pages plus one dynamic segment**: the home page, `/pricing`, **two service routes** under `/services/`, the **`/work` case study grid** and **`/work/<slug>` per case study** (decision-log #22–#26, **#30**, **#39–#41**)
...
/work/[slug]               — one route per case study, prerendered at build
                             from src/content/case-studies/ through the one
                             shared template (static, ink)                   [NEW — #39, #42]
...
**The case study routes — CHANGED, live (decision-log #39–#41, 2026-09-12).** `/work` is the grid: the `/pricing` chrome (page lockup, glow, nav, footer, metadata), an intro band reusing the proof band's own eyebrow, headline and supporting line, and every study as the band's card in its `tile` variant, on a column ladder chosen from the count so no tile is ever stranded alone on a row (`work-grid.tsx`). `/work/<slug>` is one route per study rendered by **one shared template** (`case-study-page.tsx`, **#42**, 2026-09-14), composed from an owner-supplied reference and adapted wholesale to the design system: the same chrome, then a hero (a route back to `/work`, the study's name at hero scale, its intro, the off-site links — **"Visit website" only for a live project, none is, #43** — and three hairline label/value rows), a full-width image frame, and the blocks the study's module declares in order — `text` (label, statement, paragraphs), `image` and `pair` frames — then the next two studies as "Other projects" with the band's own "View all", then the service routes' close with the study's `need` preselected. **Every frame is the layout-final placeholder until the owner supplies imagery; every string is DRAFT starter copy** grounded only in each study's summary and screenshot — no client, no numbers, no results (Rule 4.3). Content: `src/content/case-studies/` (one module per study, #41) and the page strings in `src/content/work.ts`. Adding a study is a module and its images; the template, the route and the grid do not change.
