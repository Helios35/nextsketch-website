...
**Version:** 2.6 · **Date:** 2026-09-18 · **Status:** Active — one service vocabulary across every surface, service and case study slugs double as route segments, case studies are one module each, and a case study page carries a fixed platform badge vocabulary (decision-log #27–#30, **#39–#44**)
...
**Platform badges — CURRENT (#44, owner vocabulary 2026-09-14).** A case study page's chip row is its service (the §1 name, from a `service: ServiceSlug`) plus one or more platform badges from a fixed set of six: **Web App** (`web-app`) · **SaaS** (`saas`) · **Mobile App** (`mobile-app`) · **Device** (`device`) · **Agentic Platform** (`agentic-platform`) · **Internal Tool** (`internal-tool`). Keys in `src/lib/types.ts` → `PlatformBadge`, labels once in `src/content/work.ts` → `PLATFORM_BADGES` (total over the type, so a new key without a label fails typecheck). Mascot carries `device` + `mobile-app`. Adding a value is an owner call and a decision-log row.
