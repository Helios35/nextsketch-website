...
**The hero carries an ambient gold light (owner direction, 2026-08-28).** The home page's hero glow is the warm light *in the orbit footage*; these routes mount no video (#17), so it is reproduced as a single soft `gold` radial at 19% opacity behind the hero. It is **not clipped** — the owner asked for it to bleed, so hero and the band under it share one `isolate` wrapper and the light runs across both, fading out before §How it works. Sized `inset-x-0`, so it bleeds vertically and never widens the page; `-z-10` inside the wrapper's stacking context, so it sits above the surface and below every piece of content. §Surfaces' "no glows — depth comes from shadow and blur, never from colored light" is **unchanged and still binding**: that rule governs *panel and control depth*, and nothing here glows — no card, no button, no tile. This is hero atmosphere standing in for footage, which is the one thing on `/` that was already coloured light. `gold` is the only colour used and no token was added.
