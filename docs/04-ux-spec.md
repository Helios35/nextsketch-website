...
**One hamburger and one button, every breakpoint.** The bar carries the brand lockup slot on the left and, on the right, the `NAV.featured` button beside a right-justified burger, at **all** widths; the visible desktop anchor-tab row is **RETIRED**. Everything else about the bar is unchanged: fixed, transparent over the page's first screen, elevated glass (`#0a0a0c/95 + backdrop-blur-xl`, `white/10` hairline) with `py-5 → py-3` shrink past 80px, surface on the inner div so the backdrop-filter never becomes the containing block for the overlay.
...
**Everything `/pricing` solved is reproduced, not re-derived.** Flat `ink`; nav and footer mounted by the page, never the layout, so the 404 keeps its paper surface; its own metadata; **`ScrollVideo` not mounted** (#17); and the **zero-height `sticky top-0` wordmark wrapper**, which is load-bearing and must stay the section's first child with no padding above it. Two consequences of mounting no video are handled rather than inherited, the same pair the Work band and `/pricing` handle: cards are **solid `#0a0a0c`** rather than §Surfaces' translucent-plus-blur (nothing behind them to blur), and headings carry **no text shadow** (plain ink, where §Typography bans it). Measured at build: the 80px handoff is identical to `/pricing`'s at every scroll position — page lockup only below 80px, both from 82px up.

**Four blocks, owner-specified:**

1. **Hero** — the `/pricing` intro band exactly: sticky lockup, mono eyebrow, `display`-scale `SectionHeading as="h1"` with a gold payoff word, the §05 description, and the divided-arrow `ModalTrigger`. Load-time `rise-in` at the 0 / 120 / 200ms stagger, not a scroll trigger, because it is above the fold on arrival.
2. **"What you get" — the anchored topic blocks, as side-by-side columns (owner direction, 2026-08-28, against a supplied reference).** One `<section id>` per topic, laid out as a flat multi-column band: a **top hairline**, then mono index → panel-scale name (`text-2xl → md:text-3xl`) → description → the gold-diamond deliverables list → gold `ServiceCta` text link. Column count is the block count (two on the agentic route, which is the reference's own shape; three on the product route, stepping `md:2 → lg:3`). **No card chrome** — the band reads as air against the process cards below it rather than as a second grid of the same thing. Adopted from the reference: the column count, the heading-over-list rhythm, the icon gutter and the absence of chrome. **Refused:** its four line-art glyphs (§Interaction vocabulary already settles this — "the list/label marker is a small gold diamond", and icons here are inline SVG only, so a new icon set would invent a pattern the spec covers), its light surface and grey body, and dropping the description and CTA (the §05 descriptions are the only approved copy the grouped route has, and the per-block CTA is the preselect seam). The separator is a top rule per column rather than the reference's vertical rule between them: it is the Process accordion's own device, it survives the wrap from three columns to one, and it gives the target treatment somewhere to live. `globals.css`'s `section[id] { scroll-margin-top: 5rem }` clears the fixed bar on a deep link with no extra CSS.
...
4. **Close** — `FINAL_CTA` heading with its gold payoff phrase, the divided-arrow `ModalTrigger` repeated, and one gold text link to `/pricing`. **No price on the page**; the link's label is `PRICING.headline` itself, so nothing is authored for it.

**Conversion goes through the existing seam** — `ModalTrigger` for the block-level actions, `ServiceCta` for the per-topic ones, both carrying the block's `need` into the modal already provided by the layout. Every CTA string is from the Rule 3.1 set; nothing new was added.
...
**Motion is the shipped vocabulary and nothing else — no keyframe was added and `globals.css` is untouched.** The hero entrance is the load-time `rise-in`; in the "what you get" band each column's head reveals at `i·120ms` and its rows at `i·120 + 120 + j·70ms`, so the columns fill left to right instead of in lockstep (the §Motion-inventory list rhythm, tightened because these rows are shorter than a card). The **`target:` treatment is the one new interaction**: side by side, the anchors land at nearly the same scroll offset, so arriving has to say *which* column you arrived at. The targeted column's **index goes gold** and its rule brightens `white/15 → white/40` at the 150ms micro-transition tempo — gold on the index alone is the Process accordion's open-row treatment exactly, and keeps the accent scarce on a band that already spends it on every marker. Pure CSS, so it holds with no JS and adds no transform for reduced motion to suppress.
