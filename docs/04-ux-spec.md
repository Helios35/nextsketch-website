...
2. **The anchored topic blocks** — one `<section id>` per topic, each a squared hairline glass card running mono index → panel-scale name (`text-2xl → md:text-3xl`) → description → gold `ServiceCta` text link, with the pricing card's **`Included`** list in a second column. `globals.css`'s `section[id] { scroll-margin-top: 5rem }` clears the fixed bar on a deep link with no extra CSS.
...
**Three copy slots are owner-owed and render nothing (Rule 4.3):** the one-line promise `<h1>` per page (the heading falls back to the canonical name, so nothing invented ships and the page is never headless), the `/services/product` hero intro, and the "what you get" bullets on all five blocks. The components already lay all of it out. As-built record: build-note 26.
