...
1. **Hero — CHANGED (owner direction, 2026-08-30, against a supplied reference).** Reproduces `/pricing`'s sticky lockup exactly, then three things the reference contributed:
   ...
   - **Two-up from `lg:`** — eyebrow, `display`-scale `<h1>`, description and CTAs on one side; the illustration on the other, behind a `lg:border-l` column rule. Below `lg:` it stacks text-first. (The `hero` type scale was considered and **not** adopted: the one-line promise is still owner-owed and the canonical-name fallback would dominate at `lg:text-8xl`.)
   ...
   **The illustration is a wireframe on a tilted plane** (`service-hero-visual.tsx`) — a workflow graph on the agentic route, a layered product UI on the product route. **Perspective is net-new vocabulary and is a flagged judgment call:** the repo had no `perspective` / `rotateX` / `preserve-3d` / `skew` anywhere, and the only rotation in the system is the `rotate-45` gold diamond. Everything *on* the plane is existing vocabulary — squared hairline cards on solid `#0a0a0c`, skeleton matter on the white alpha ladder, chrome tints from #31, one gold element, the mocks' radial fade — and only the tilt is new. Written as arbitrary properties, not Tailwind 3D utilities. A static transform is layout, not motion, so nothing is gated on reduced motion and **no keyframe was added**. **Rule 4.3:** no readable string, numeral, metric or logo, and the whole visual is `aria-hidden`. It is **contained and faded, not bled** off the page edge — a rotated plane overflowing horizontally widens the page, and the usual fix (`overflow-x-clip`) would sit above the `sticky` lockup and risk the 80px handoff.
