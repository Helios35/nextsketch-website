...
## Addendum — Agentic Platform's imagery lands (owner direction, same day)

The owner pointed at an Engen folder for the Agentic Platform study (Engen is the product; the study keeps its published title, build-note 20) and set both ends: the laptop render is the hero and the interface overview ("UI Intro") is the last image. The attached folder (`__WORK/2026/Engen/`) held one 574 × 112 toolbar crop and nothing else; the set the owner described is in `__WORK/2025/Engenai/Portfolio/`, twelve files, and that is what shipped from — flagged here rather than silently substituted.

Six of the twelve, in page order: `agentic-platform-01.png` the laptop (the owner's hero; 6144 × 3072, **4.4 MB**, the heaviest asset in the repo and the one to downsample if the repo size matters) · `02` and `03` the two workflow diagrams as the pair · `04` the workspace with the agent prompt open · `05` the workspace on the properties tab · `06` the interface overview (the owner's closer). Left out, and a `src` swap away: the intro board (the hero's laptop again under a title) and four more workspace states that differ only by which panel is open. Every frame keeps the template's ratio (owner rule) and the module doc block says which 2:1 shots went where and why. 7.8 MB together, shipped as supplied.

What the fixed frames crop, for the owner's eye: the `16/9` hero and the two workspace states are 2:1, so each loses a sliver of width and nothing that matters; the `4/3` pair trims the top and bottom of the two diagrams (a title line and a caption each); **the closing overview is a tall 2:3 board, so the `16/9` frame shows its middle third** — the dark UI-kit panel and the top of the component sheet, not the "Interface" title above or the bottom of the sheet. That is the rule applied as stated; a wider crop of that board is the fix if the owner wants more of it. Also worth a look: the two diagrams in the pair carry the same introductory paragraph, so side by side they read as a repeat. Verified in the browser: all six load, frames at 1.778 / 1.333 × 2 / 1.778 × 3, no overflow, no placeholder left on the route.

**Still on placeholders:** SaaS Platform and Parcell. The pattern is set: a folder of renders, named into `public/work/<slug>/`, one `src` line per slot.
