...
**Video asset:** `Consulting Video 1.mp4` copied from the workspace redesign folder into **`public/consulting-video.mp4`**, then **re-encoded for scrubbing** after the owner flagged chop (ffmpeg via a scratchpad-local `ffmpeg-static` — nothing installed system-wide): H.264 all-progressive with a **keyframe every 4 frames** (`-g 4 -sc_threshold 0`), audio stripped, 1280×720 @ 24fps preserved, 2.7 MB (vs 2.2 MB source). Dense keyframes mean any seek decodes ≤3 frames — this, plus gating `currentTime` writes on `video.seeking` so seeks never stack, is what makes the scrub read smooth. Measured post-fix: frame updates saturate a 20 Hz sampler during continuous scroll; sync stays exact (settles at 4.92s of 9.87s at half-scroll).

## What each section became (Phase B — reworked in place, existing copy reused)

All five are transparent full-width bands over the backdrop: binding gutters (`px-6 sm:px-8 lg:px-16`), mono `(0n)` eyebrow via `SectionHeading`, one gold payoff phrase per headline (presentation-marker pattern — degrades if canonical copy changes), display headings carrying the licensed over-imagery text shadow. **No divider hairlines between sections** (owner direction, 2026-07-06 — the band-opening `border-t white/10` and the footer's top hairline were removed so the footage flows uninterrupted; hairlines remain component vocabulary inside the accordion, cards, and panels).
...
6. `NAV.cta` is now unused by the nav (kept as the canonical Rule-3.1 string for the hero/final CTAs) — say the word if you want it pruned or repointed.
7. **Open the PR when verified** — branch pushed, no PR per policy.
