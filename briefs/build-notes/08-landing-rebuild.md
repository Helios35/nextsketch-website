...
- **Premium modal material (2026-06-14 follow-up, owner-asked via `frontend-design` + `ui-ux-pro-max`):** the color-only retheme above wasn't enough, so the ui-ux-pro-max **Modern Dark / Cinema** depth system was applied **to the modal**: its surface is **never pure `#000`** (an elevated near-black `#0b0b0d`/`#141416` top-lit gradient card with a lit hairline border + deep `--shadow-modal`), option rows are raised cards that fill gold **with a gold glow** when selected, inputs gain a gold focus ring, and the shared `<Button>` now lifts on hover / compresses on press (the white `inverse` warms to a gold glow) — all on one shared expo-out easing (`cubic-bezier(0.16,1,0.3,1)`), backdrop frosted (`blur(8px)`). Quality floor: transforms `motion-safe`-gated, focus rings preserved. **Note:** a first attempt over-reached and also regraded the hero (gold ambient light, grain, a gold-chip CTA); **reverted at owner request** — the hero stays the approved template treatment (divided-arrow CTA, simple overlay), and the premium work is confined to the modal.
