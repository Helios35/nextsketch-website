...
- **Premium material pass (2026-06-14 follow-up, owner-asked via `frontend-design` + `ui-ux-pro-max`):** owner flagged the CTA as flat / hard-edged and the modal as "just colors." Applied the ui-ux-pro-max **Modern Dark / Cinema** system — surfaces are **never pure `#000`** (elevated near-blacks `#0b0b0d`/`#141416` + a top-lit gradient), **lit hairline borders**, one shared **expo-out easing** (`cubic-bezier(0.16,1,0.3,1)`) on every transition, and a depth/glow token set in `globals.css` (`--shadow-cta`, `--shadow-cta-hover`, `--shadow-modal`). The **CTA is the signature**: a warm-white gradient pill (`rounded-2xl`) with a gold "go" chip, a layered shadow **+ gold glow**, hover-lift + arrow-advance, press-compress. Hero gains cinematic atmosphere — layered near-black scrim, a **gold ambient light** tying the accent to the scene, vignette, and **filmic grain** (`.grain-overlay`). Modal card is an elevated near-black gradient with a deep shadow; option rows are raised cards that fill gold **with a gold glow** when selected; inputs + buttons gained depth and premium states (shared `<Button>` now lifts on hover / compresses on press; `inverse` warms to a gold glow). Quality floor held: all transforms `motion-safe`-gated, focus rings preserved, contrast checked.
