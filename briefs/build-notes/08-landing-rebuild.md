...
- **Modal matched to the template (2026-06-14 follow-up; owner: "make the modal match… this is just a visual change of the modal, do not change anything else"):** earlier passes recolored, then imposed a soft/rounded/glowy "premium" look that still didn't match. This pass adopts the **template's actual visual language** (the hero CTA + marquee): **squared corners** (`rounded-none`), **hairline borders** (`white/12–15`), a **glassy** translucent near-black surface (`bg-[#0a0a0c]/95 backdrop-blur-xl`), **JetBrains-Mono uppercase micro-labels** (field labels, escape hatch, failure list), a **thin segmented progress meter** (replacing round dots), **flat gold** selection (gold fill + paired gold-ink text, no glow), and the **divided-arrow button** matching the hero CTA for every forward action (Next / submit / send) with a squared hairline ghost for Back. **Hover:** advance buttons scale subtly and nudge the arrow (smooth — transitioning the Tailwind-v4 standalone `scale`/`translate` properties, not `transform`), the ghost shifts its border/background. The modal carries its **own squared buttons** (`AdvanceInner` + `ADVANCE_CLASS`/`GHOST_CLASS`) instead of the shared pill `<Button>`, so **`button.tsx` was reverted to its pre-premium state** — nothing outside the modal changed (verified: diff vs the approved state is `qualification-modal.tsx` + the modal-only globals tokens, full stop). Flow / validation / a11y logic untouched; focus rings preserved, reduced motion respected. (For the record: two earlier missteps — a color-only retheme, then a soft-glow pass that over-reached into the hero and was reverted — before the modal was re-scoped and matched to the template.)
