...
| Card | Destination |
|---|---|
| New Product | `/services/product` |
| Product Completion | `/services/product` |
| Product Support | `/services/product` |
| Agentic System | `/services/agentic-system` |

**All four open their route at the top — corrected 2026-08-30.** The three product cards originally carried block anchors, which is what the 2026-08-28 direction asked for. It was settled before the routes had a hero worth landing on: measured from `/`, a card put the visitor at **scrollY 544 with the `<h1>` 199px above the fold**, so the ribbon, the heading, the CTAs and the graphic were all skipped on arrival. The agentic card never had this problem because it always pointed at its route root — which is why the fault showed on one page and not the other.

Worth recording, because it looked like a second bug and was not: a repeat visit can still land mid-page. That is the browser restoring scroll for a URL already scrolled in that tab (`history.scrollRestoration` is `auto`), verified in a fresh tab where the route loads at scrollY 0 with the hero in full. The block anchors are untouched and still resolve for a shared or bookmarked URL; nothing on the site generates one any more, and `serviceBlockHref` stays exported so the next caller does not hand-write the hash this unit exists to prevent.
...
`service-hero-visual.tsx` — a workflow graph on the agentic route (the reference's own subject), and on the product route **two overlapping product surfaces**: a column chart behind, a task list lifted in front of it. Same tilt, so the two heroes read as one system.

The product graphic was rebuilt against a second owner-supplied card reference (2026-08-30, product route only — the agentic graph is untouched), then **mirrored and given a stronger camera** at the owner's direction: the composition flips horizontally (`rotateZ` sign inverted, card positions mirrored), the two cards overlap further for depth, and the camera moves in (`perspective` 1500px → 1050px, `rotateX` 32° → 40°). Perspective and tilt are now **per route** rather than shared constants, so the two heroes are deliberately no longer identical and neither can be changed by editing the other. Structure adopted: the two overlapping cards, the header-plus-control row, the column chart, the sub-panelled task rows with a status dot and an avatar. Refused: **the labels and figures.** The reference names every column and prints a due figure on every row; reproducing that puts invented metrics on the page a search visitor lands on first, which is what Rule 4.3 and decision #5 (the retired stat strip) exist to stop. The chart has **no axis, no scale, no numerals and no day labels** — varying bars with nothing written on them assert nothing, the same posture the ops panel's uptime strip already ships. Also refused: its rounded corners and light surface, and its photographic avatars, which become the system's profile chip because a portrait would be an invented person. `gold` stays singular — the peak column, and nothing else; the row dots are chrome tints (#31).
...
- **The fade must be an explicitly sized ellipse.** `ellipse at center` with no size defaults to `farthest-corner`, which puts the gradient's 100% at the **corners**. On the hero's 4/3 box that left the left and right mid-edges at 80% of the radius and the top and bottom at 60%, so a 46% → 97% ramp reached only ~67% ink on the sides and ~27% top and bottom — a hard clip on two edges and a weak one on the others (owner report, 2026-08-30). `ellipse 50% 50%` sizes the radii to the box's half-width and half-height instead (radial-gradient size percentages resolve against the corresponding box dimension), so the last stop lands exactly on all four mid-edges and the corners sit past it at ~141%. **Do not drop the explicit size** — it looks like noise and is the entire fix.
